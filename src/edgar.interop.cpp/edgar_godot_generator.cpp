...
Ref<EdgarGodotGenerator> EdgarGodotGenerator::cons(Vector2i from, Vector2i to) {
...
void EdgarGodotGenerator::_bind_methods() {
    ...
    ClassDB::bind_static_method(get_class_static(), D_METHOD("cons", "from", "to"), &EdgarGodotGenerator::cons);
}