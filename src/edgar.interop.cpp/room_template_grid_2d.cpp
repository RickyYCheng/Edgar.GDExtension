...
Ref<RoomTemplateGrid2D> RoomTemplateGrid2D::cons(StringName name, PackedVector2Array outline, Ref<ManualDoorModeGrid2D> doors, PackedInt32Array transformations) {
    RoomTemplateGrid2D *self = memnew(RoomTemplateGrid2D);
    auto name_buffer = name.to_utf8_buffer();
    self->csharp_obj_handle = csharp_obj_alloc_room_template_grid_2d(&name_buffer, name_buffer.size(), &outline, outline.size(), doors->get_csharp_obj_handle(), &transformations, transformations.size());
    return self;
}
